...
// URL pointing to the antivirus / SmartScreen troubleshooting section in the
// project README. Every fatal-install message links here so users have a
// single, authoritative place to read remediation steps.
const HELP_URL_AV =
...
function scheduleReconnect(reason) {
  ...
  if (state.nativeHostFatal) {
    log(`Not reconnecting — fatal error from native host (${state.nativeHostFatalCode}). ` +
        `See ${HELP_URL_AV} for how to fix this. ` +
...
function friendlyFatalMessage(code, raw) {
  // All friendly messages end with a link to the README troubleshooting
  // section so users have one canonical place to read the fix.
  const help = ` See ${HELP_URL_AV} for step-by-step instructions.`;
  switch (code) {
    case 'WIN_KEY_SERVER_MISSING':
      return 'WinKeyServer.exe is missing from the install. This is almost ' +
             'always caused by antivirus software (Microsoft Defender, ' +
             'CrowdStrike, SentinelOne, etc.) quarantining the file.' + help;
    ...
    case 'KEY_LISTENER_SERVER_ERROR':
      return 'Native host could not start the global key listener. ' +
             'This is usually an antivirus blocking WinKeyServer.exe.' + help;
    case 'NOT_REGISTERED':
      return 'Native messaging host is not registered. Run install.bat.' + help;
    default:
      return (raw || 'Native host failed: ' + (code || 'unknown')) + help;
  }
}
